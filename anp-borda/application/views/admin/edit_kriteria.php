			...
			<div class="col-md-12">
				...
				<div class="portlet box grey-cascade">
					...
					<div class="portlet-body">
						<?= form_open_multipart('admin/edit-kriteria/' . $id) ?>
							<div class="form-group">
								<label for="kriteria">Nama Kriteria</label>
								<input type="text" name="kriteria" value="<?= $kriteria->kriteria ?>" class="form-control">
							</div>
							<div class="form-group">
								<label for="bobot">Bobot</label>
								<input type="number" name="bobot" value="<?= $kriteria->bobot ?>" class="form-control">
							</div>
							<div class="form-group">
...
<script type="text/javascript">
	let subkriteriaCount = 0;

	<?php foreach ($kriteria->subkriteria as $subkriteria): ?>
		add_subkriteria('<?= $subkriteria->subkriteria ?>');
	<?php endforeach; ?>

	function add_subkriteria(val) {
		if (val == undefined) {
			val = '';
		}
		$('#subkriteria-container').append('<tr>' +
				...
				'<td>' +
					'<input type="text" value="' + val + '" class="form-control" name="subkriteria[]" required>' +
				'</td>' +
